...
Before running on a new machine, verify the local-only files that are not
stored in git:

```text
dataset/ieee_cis/train_transaction.csv       required to rebuild IEEE-CIS splits
dataset/ieee_cis/train_identity.csv          required to rebuild IEEE-CIS splits
dataset/processed/client_*/...parquet        generated locally, not pushed
outputs/checkpoints/<run>/round_*.pt         generated locally, not pushed
results/<run>/                               generated locally, not pushed
app/node_modules/                            generated by npm install
.venv/                                       generated by uv sync
```

If processed data or checkpoints are missing, that is expected after clone.
Rebuild data from the raw CSV files, then train or copy a compatible checkpoint
