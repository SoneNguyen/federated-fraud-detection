...
def gen_rows(n: int, is_fraud: bool, rng: np.random.Generator, currency: str, tz_str: str) -> dict[str, list]:
...
def main() -> None:
    for cfg in CLIENT_CFG:
        ...
        print(
            f"Client {cfg['id']} ({cur}/{cfg['tz']}): {len(df):,} rows, "
