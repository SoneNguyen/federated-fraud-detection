...
def _make_feat_report(severity="WARNING", stale=0.1):
    psi = {
        "tx_amount_usd": 0.15, "tx_count_1h": 0.01, "tx_count_24h": 0.01,
        "tx_volume_1h_usd": 0.01, "tx_volume_24h_usd": 0.01,
        "merchant_cat_dev": 0.01, "geo_velocity_kmh": 0.01,
        "days_since_last_tx": 0.01, "account_age_days": 0.01,
    }
    return DriftReport(
...
def test_push_feature_sets_psi_gauges():
    ...
    report = _make_feat_report()
    # Should not raise — gauges accept any float
    exporter.push_feature(report)


def test_push_feature_sets_severity_gauge():
    from drift import exporter
    for sev, expected in [("INFO", 0), ("WARNING", 1), ("CRITICAL", 2)]:
        report = _make_feat_report(severity=sev)
        exporter.push_feature(report)
        assert exporter.SEV[sev] == expected


def test_push_feature_sets_stale_fx_rate():
    from drift import exporter
    report = _make_feat_report(stale=0.75)
    exporter.push_feature(report)  # should not raise


def test_push_prediction_sets_shift_gauge():
    from drift import exporter
    report = _make_pred_report(shift=0.12)
    exporter.push_prediction(report)  # should not raise


def test_push_auprc():
    from drift import exporter
    exporter.push_auprc(0.82)  # should not raise
